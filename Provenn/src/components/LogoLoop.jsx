...
export const LogoLoop = memo(({
  ...
  // Define a cor do fade automaticamente para dark
  const cssVariables = useMemo(() => {
    let fadeColor = fadeOutColor ?? '#ffffff';
  ...
  const renderLogoItem = useCallback((item, key) => {
    ...
    const inner = item.href ? (
      <a className="inline-flex items-center no-underline rounded transition-opacity duration-200 ease-linear hover:opacity-80 focus-visible:outline focus-visible:outline-current focus-visible:outline-offset-2" href={item.href} aria-label={itemAriaLabel || 'logo link'} target="_blank" rel="noreferrer noopener">
        {content}
      </a>
    ) : content;
